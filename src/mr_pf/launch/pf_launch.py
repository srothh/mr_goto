...
def params_path(context: LaunchContext):
    path_name = "$MR_DIR/ws02/install/mr_pf/share/mr_pf/config/" + context.launch_configurations()
    context.perform_substitution(path_name)

def map_path(context: LaunchContext):
    path_name = "$MR_DIR/ws02/install/mr_pf/share/mr_pf/config/" + context.launch_configurations()
    context.perform_substitution(path_name)
...
def generate_launch_description():
    ...
    OpaqueFunction(function=params_path, args=['particle_filter_params_file'])
    OpaqueFunction(function=map_path, args=['particle_filter_map_file'])
